...
const CATEGORIES = [
  ...
  { min: 25, max: 30, label: 'OVERWEIGHT' as const },
  { min: 30, max: Infinity, label: 'OBESITY' as const },
];
...
export class BmiService {
  calculate(weightKg: number, heightCm: number) {
    ...
    const cat = CATEGORIES.find(c => value >= c.min && value < c.max)?.label ?? 'OBESITY';
  ...
  async createRecord(userId: string, weightKg: number, heightCm: number) {
    ...
    const record = await prisma.bmiRecord.create({
      data: { userId, weight: weightKg, height: heightCm, bmi: value, bmiCategory: category === 'Kelebihan Berat' ? 'OVERWEIGHT' : category === 'Kurus' ? 'KURUS' : category === 'Normal' ? 'NORMAL' : 'OBESITY' },
    });
  ...
  async getAll(query: { page?: number; limit?: number; search?: string }) {
    ...
    return {
      records: records.map(r => ({ id: r.id, user: { name: r.user?.name, email: r.user?.email }, weightKg: Number(r.weight), heightCm: Number(r.height), bmiValue: Number(r.bmi), bmiCategory: LABELS[r.bmiCategory], createdAt: r.createdAt.toISOString() })),
      pagination: { page, limit, total, totalPages: Math.ceil(total / limit) },
    };
  }

  private format(r: { id: string; weight: unknown; height: unknown; bmi: unknown; bmiCategory: string; createdAt: Date }) {
    return { id: r.id, weightKg: Number(r.weight), heightCm: Number(r.height), bmiValue: Number(r.bmi), bmiCategory: LABELS[r.bmiCategory], createdAt: r.createdAt.toISOString() };
  }
}
